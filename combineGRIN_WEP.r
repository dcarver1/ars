...
sosTotalUnique <- nrow(unique(grinData[1]))
...
joinedSOS_WEP <- left_join(grinData,slimWEP,by="Taxon")
head(joinedSOS_WEP)
...
print(paste( "There are a total of ",
             numUnique, " unique taxons collected by SOS and stored by ARS that are also",
             "defined as World Econimic Plants.."))


print(paste( percentSOS, "percent of all the ARS stored SOS collections are reconized as",
             "defined as World Econimic Plants."))
...
labelsWEP_SOS <- c("total Taxons", "total collected and Stores", "percent of total" )
labelsColumns <- c('WEP Taxons', "SOS ARS Taxons")
