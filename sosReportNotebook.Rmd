...
allGroups
ggplot(allGroups, aes(x=year,  y=cumsum, color = use, alpha=0.8)) +
  geom_point(size=3)+
...
ordersDF <- date[!duplicated(date$order_request_id),]


#define function to produce unique DF for use use code 
cumulativeFun <- function(dataFrame, useCode) {
  # creates a cumulative count of the orders based on use code
  #returns a datatable 
  df1 <- dataFrame %>%
  filter(dataFrame$intended_use_code == useCode )%>% 
  group_by(year)%>%
...
#list of unique uses 
uses <- unique(ordersDF$intended_use_code)

# initalize empty DF 
speciesOr2 <- data.frame()

# itorate trough use list to apply function and build a complete dataframe 
for(use in uses){
  df1 <- cumulativeFun(ordersDF, use)
  speciesOr2 <- bind_rows(speciesOr2,df1)
}


# plot data 
ggplot(speciesOr2, aes(x=year,  y=cumsum, color = use)) +
  geom_point(size=3)+
...
intendedUse2 <- date[!duplicated(date$order_request_id),] %>%
  filter(intended_use_code %in% c( "RESEARCH")) %>%
  separate(intended_use_note, c("Secondary_Class", "Research_use"), "Research use notes")

# list of all possible secondary uses for reserach (gathered from GRIN )
...
#define function to produce unique DF for use use code 
cumulativeFun2 <- function(dataFrame, useCode) {
  # creates a cumulative count of the orders based on use code
  #returns a datatable 
  df1 <- dataFrame %>%
  filter(dataFrame$Secondary_Class == i)%>% 
  group_by(year)%>%
  dplyr::summarise(number =n()) %>%
  mutate(cumsum = cumsum(number))%>%
  mutate(use = useCode) 
...
for(i in researchUses){
  df1 <- cumulativeFun2(intendedUse2, i)
  research <- bind_rows(research,df1)
}


ggplot(research, aes(x=year,  y=cumsum, color = use)) +
  geom_point(size=3)+
...
education <- date[!duplicated(date$order_request_id),] %>%
...
education2 <- data.frame()

# itorate trough use list to apply function and build a complete dataframe 

for(i in educationUses){
  df1 <- cumulativeFun2(education, i)
  education2 <- bind_rows(education2,df1)
}


ggplot(education2, aes(x=year,  y=cumsum, color = use)) +
  geom_point(size=3)+
