...
subsetOrder <- cleanOrders %>%
  select("Order.Request.ID", "Order.Type","Intended.Use","Intended.Use.Note","Item.Count","Ordered.Date","Completed.Date" , "Note" )
...
research <- intendedUse2 %>%
  filter(Intended.Use %in% c( "Research")) %>%
...
education <- intendedUse2 %>%
  filter(Intended.Use %in% c( "Education")) %>%
...
trimList <- read.csv("commonWords1.csv") %>%
  ...
  arrange(desc(Number.of.Occurances))# removes undescroptive common words 

# print the full list for viewing 
...
allText <- intendedUse2 %>%
  select("Intended.Use", "Secondary_Class","qualData" )
...
### Everything Below here many need so reworking of the datasets 
...
print("data came from SOSOrdersTaxa.csv")
...
print( "data is from SOSOrdersTaxa.csv")
```
<br>
<br>
...
DT::datatable(accessionsSent)

print("data is from sosOrderItems.csv")
```
...
taxaSent1 <- speciesOr %>%
  ...
  arrange(desc(count))
DT::datatable(taxaSent1 %>% arrange(desc(count)) %>% `colnames<-`(c("Taxon", "Total Delivered", "Total Packets")))
...
DT::datatable(resquestor1 %>% arrange(desc(count)) %>% `colnames<-`(c("Taxon", "Total Taxon", "Total Delivered", "Total Packets")))
