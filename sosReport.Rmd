...
# percentage of SOS accession in this class
percentWUS <- signif(100*(nrow(wusSamples) / totalSamples),3)
...
#percent of sos in this class
percentCWR <- signif(100*(nrow(cwrSamples) / totalSamples),3)
...
single <- WEPData %>% distinct(Taxon)

combinedData2a <- inner_join(sosData, single, by='Taxon', type='left', match='all')
...
#percent of sos in this class
percentWEP<- signif(100*(nrow(combinedData2a) / totalSamples),3)
#calculate the total number of accession that are either or 

## DEfeine row and column names 
labelsRow <- c("SOS Collection")
labelsCol <- c('All Accession', "Total Number of Taxon", "Unique Number of WUS Taxa from K13", "Percent of Accessions that are WUS", "Unique Number of CWR taxa from K13", "Percent of Accessions that are CWR", "Unique Number of taxa WEP" , "Percentage of Accessions that are WEP")

#this is a bit sloppy but I'm gonig to role with it for the moment I wanted to show a table before showing the data from this section. The trouble is I'm pulling variables from this section of code. When I get time I should polish this up 
```
...
# create the first row of the dataframe 
sosData <- c(uniqueFam, uniqueGen, uniqueSpe)
# create K13 WEP/CWR 
K13wus <- c( n_distinct(wusSamples$FAMILY), n_distinct(wusSamples$GENUS), n_distinct(wusSamples$SPECIES))
K13cwr <- c( n_distinct(cwrSamples$FAMILY), n_distinct(cwrSamples$GENUS), n_distinct(cwrSamples$SPECIES))
...
WEP <- c( NA, as.numeric(n_distinct(WEPgs$GENUS)), as.numeric(n_distinct(WEPgs$SPECIES)))
...
#define column names and row names 
colnames(dataSummary) <- c( "Total Number of Familes", "Total Number of Genus", "Total Number of Species")
