...
- at this stage the majority of the effort of the authors has been placed into developing a series of graphic/tables that summarizes the relationship between the SOS and ARS by address 6 primary questions (See results). 
...
- background on both the organizations needs to be added based on the final audience 
...
2.  How many of the SOS accessions are considered CWR, WUS, or WEP
3.  How many of the SOS accessions are mainted on ARS systes (GRIN and backed up at NLGRP) how does this effect the diversity and usefullness of the collection. 
4.  Within the ARS which locations are resposible for the SOS accessions 
5.  How successful is the ARS in germinating the seeds provides by the SOS program 
6.  Who is using the SOS accessions and what for 
...
This illustrates that the rate of accumulation between the three groups flucuates a bit over time. The drop in number of samples incorperate into GRIN and NLGRP in the more resent years is due to the 
...
It is unclear if the reason for distribution column within the grin global accession request process is a fill in the blank or drop down. There is a lot of variablility within the supplied answers. **Should requestors be able to enter their own reason for a request** 
...
reciever <- distribution %>%
  ...
  dplyr::rename("Entity Recieving Seeds" = 'title'  ,"Number Of Genus Distributed" = 'numberOfGenus', "Samples Distributed" = 'numberOfSeeds')

reciever

```

Table illustrating the total number of seeds sent per genus. There has been a total of `r totalSeedsSent` seeds and or packets distributed by the ARS that were collected by the SOS program.
**At the same time I'm a little wary of this data count. why would someone order a single seed. 2/3 of all order are under 10 seeds. Does that make sense** 
...
**again the question of the base unit is hear. It would seem that a packet is the smallest unit counted but I do not know** 
